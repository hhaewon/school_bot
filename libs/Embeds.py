...
class Embeds:
    ...
    async def meal_service(
            cls,
            params: Sequence[RequestParameters],
            now_date: datetime,
            date: datetime,
            school_name: str,
    ):
    ...
    async def school_schedule(
        ...
        embed = Embed(
            ...
            description=f"{school_name}의 {formatted_term}의 학사일정",
        )

        try:
            school_schedule_response = await SchoolApi.request_school_schedule(
                params=params
            )
            school_schedule_info = "\n".join(
                f"{day} : {name}"
                for name, day in school_schedule_response.schedule.items()
            )
    ...
    @classmethod
    async def school_schedule_notification(
            cls, params: RequestParameters, school_name: str, now_date: datetime
    ):
        formatted_now_date = now_date.strftime("%Y/%m/%d")
        embed = Embed(
            title="학사일정", colour=Colour.random(), description=f"{school_name}의 오늘의 학사일정"
        )
        try:
            school_schedule_response = await SchoolApi.request_school_schedule(
                params=params
            )
            school_schedule_info = school_schedule_response.schedule2[
                formatted_now_date
            ]
            embed.add_field(name="학사일정", value=school_schedule_info)
    ...
    async def adding_notification(cls, user_name: str, data: dict[str, Any]):
        embed = Embed(
            title="설정한 알림들", colour=Colour.random(), description=f"{user_name}의 설정한 알림들"
        )
