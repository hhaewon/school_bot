    ...
    def schedule2(self):
        ...
        for key, schedule in self.schedule.items():
            if "~" not in schedule:
                ...
                from_date_str: str
                to_date_str: str
                from_date_str, to_date_str = schedule.split(" ~ ")
                from_date: datetime.datetime = datetime.datetime.strptime(
                    from_date_str, "%Y/%m/%d"
                )
                to_date: datetime.datetime = datetime.datetime.strptime(
                    to_date_str, "%Y/%m/%d"
        ...
        for row in self.rows:
            ...
            if row.EVENT_NM in ["여름방학", "겨울방학"]:
                if row.EVENT_NM not in schedule_dict:
                    schedule_dict[
                        row.EVENT_NM
                    ] = f"{self._format_date(row.AA_YMD)} ~ {self._format_date(row.AA_YMD)}"
                else:
                    schedule_dict[
                        row.EVENT_NM
                    ] = f"{schedule_dict[row.EVENT_NM][:10]} ~ {row.AA_YMD[:4]}/{row.AA_YMD[4:6]}/{row.AA_YMD[6:8]}"
            else:
                schedule_dict[row.EVENT_NM] = self._format_date(row.AA_YMD)

        return dict(sorted(schedule_dict.items(), key=lambda x: x[1]))
