...
async def meal_service(context: ApplicationContext,
    ...
    try:
        ...
        embed = Embed(title="급식", colour=Colour.random(), description=f"{school_name}의 {date.strftime('%Y년 %m월 %일')}의 급식")
...
async def time_table(context: ApplicationContext,
    ...
    try:
        ...
        embed = Embed(title="시간표", colour=Colour.random(), description=f"{school_name}의 {date.strftime('%Y년 %m월 %일')}의 시간표")
